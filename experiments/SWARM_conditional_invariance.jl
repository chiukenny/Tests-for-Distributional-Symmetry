...
if train_kernel
    kci = KCI(GV_KCI, B=B)
    σxs = collect(LinRange(0.001, 3, 31))
    σys = collect(LinRange(1, 15, 57))
...
for (sym,d) in [("X_inv",1), ("Y_inv",2), ("Z_inv",3)]
    ...
    push!(results, run_tests(output_file, sym, tests, N=N, n=n, α=α, 
                             f_sample_data=SWARM_sample_data_d, f_sample_tr_data=SWARM_tr_sample_data_d))
end
...
push!(results, run_tests(output_file, "NP", tests, N=N, n=n, α=α,
                         f_sample_data=SWARM_sample_data_NP, f_sample_tr_data=SWARM_tr_sample_data_NP))
